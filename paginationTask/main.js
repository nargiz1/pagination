...
let buttonsArea = document.querySelector(".buttons-wrapper");
...
for (let i = 0; i < 10; i++) {
    if(i != 0 && i<= Math.ceil(products.length / 10)) createButton(i);
  createRow(
    products[i].id,
    products[i].name,
...
document.querySelectorAll("button").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    let currentSkip = num === 1 ? 0 : 10 * (num - 1);
    console.log(currentSkip);
    let currentTake = currentSkip + 10;
    console.log(currentTake);
    table.innerHTML = "";
    productsToShow = products.slice(currentSkip, currentTake);
    productsToShow.forEach((prod) => {
      createRow(prod.id, prod.name, prod.unitPrice, prod.unitInStock);
    });
  });
});
...
function createRow(id, name, price, unitInStock) {
  ...
  let unitInStockCol = document.createElement("td");
  unitInStockCol.innerText = unitInStock;

  row.append(idCol);
  row.append(nameCol);
  row.append(priceCol);
  row.append(unitInStockCol);
  table.append(row);
}
